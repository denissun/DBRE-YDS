...
{
cat $SCRIPTDIR/infoall_operations.${host_name}.dat | sed '/^$/d' | while read program  status  group  lag   chkpt
do
...
cat $SCRIPTDIR/stats_operations.${host_name}.dat | nawk -v hostname=$host_name -v ctime="$create_time" ' 
BEGIN {
   operations=0;
   inserts=0;
   updates=0;
