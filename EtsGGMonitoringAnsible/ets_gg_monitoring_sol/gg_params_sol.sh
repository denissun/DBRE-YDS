#/bin/bash
# gg_params_sol.sh 
#   - load gg params as lines into the respository database
#
# Author: Yu (Denis) Sun
#
# Notes:
...
{
ps -ef | grep mgr.prm | grep -v grep | grep -v gguser2 | awk '{ if ($11 ~ /mgr/ ) {print $11} else if ($10 ~ /mgr/) { print $10} }' |sed 's/dirprm\/mgr.prm/ggsci/' | nawk NF | while read command
do
...
}  > $SCRIPTDIR/infoall_params.dat


{
cat $SCRIPTDIR/infoall_params.dat | nawk NF | while read program  status  group  lag   chkpt
do
...
then
  ...
  echo " view params $group " | $GGHOME_GGSCI  | grep -v GGSCI | nawk NF | nawk  '{ if (NR>5) { print $0} } END { print "-- END OF PARAMS" } ' | cat -n  
  continue
fi
...
then
  ...
  echo " view params mgr " | $GGHOME_GGSCI  | grep -v GGSCI | nawk NF | nawk  '{ if (NR>5) { print $0} } END { print "-- END OF PARAMS" } ' | cat -n  
fi

done
}  > $SCRIPTDIR/gg_params.dat


cat $SCRIPTDIR/gg_params.dat | nawk -v hostname=$host_name -v ctime="$create_time" ' 
{
   if ( $0 ~ /^PARAMS STARTS / ) {
      gghome=$3;
      program=$4;
...
}  ' > $SCRIPTDIR/insert_gg_params.sql
...
spool $SCRIPTDIR/insert_gg_params.spool
