...
/u01/app/venv_python36/bin/ansible-playbook -i inventory -e "HOSTS=${CURRENTHOST}"  --become-method=pbrun --become-user=oracle -b  drop_user_180day_aft_rvw.yml
...
## echo "###  save the html report file to dbaets application server "
## 
for fn in $(find  logs/ -name "*.spool -mmin -20 ");
do
  echo $fn
done
## 
