#!/bin/bash
#
# Script: config_inactive_accttime.sh 
# Usage:  config_inactive_accttime .sh
#
...
TS=`date +%Y%m%d%H%M`
OUTFILE=config_inactive_accttime_${TS}.log
...
TMPOUTFILE=lock_user_90day_inactive_tmp.out
...
then
    echo "is CDB"
    # get pdb names
sqlplus -s / as sysdba <<EOF  | grep -v "^$" > $TMPOUTFILE
    set echo off head off trimspool on
    set pages
...
$SQLPLUSCOMMAND <<EOF
spool config_inactive_accttime_${INSTANCE_NAME}_$pdbname_${HOSTNAME}_$TS.spool
...
$SQLPLUSCOMMAND <<EOS
spool config_inactive_accttime_${INSTANCE_NAME}_${HOSTNAME}_$TS.spool
